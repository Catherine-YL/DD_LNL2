...
from .utils_noise import download_url, check_integrity, multiclass_noisify
...
class CIFAR100n_coarse(CIFAR10n):
    """`CIFAR100 <https://www.cs.toronto.edu/~kriz/cifar.html>`_ Dataset.
    ...
    def load_label(self):
        ...
        noise_label = torch.load(self.noise_path)
        if isinstance(noise_label, dict):
            if "clean_coarse_label" in noise_label.keys():
                clean_label = torch.tensor(noise_label['clean_coarse_label'])
                assert torch.sum(torch.tensor(self.train_labels) - clean_label) == 0  
                print(f'Loaded {self.noise_type} from {self.noise_path}.')
                print(f'The overall noise rate is {1-np.mean(clean_label.numpy() == np.array(noise_label[self.noise_type]))}')
            return np.array(noise_label[self.noise_type]).reshape(-1)  
        else:
            raise Exception('Input Error')