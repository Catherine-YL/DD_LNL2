...
def noisify_multiclass_asymmetric_cifar10(y_train, noise, random_state=None, nb_classes=10):
    ...
    if noise > 0.0:
        # Construct the flipping matrix P
        for s, t in zip(source_class, target_class):
            P[s, s] = 1.0 - noise
            P[s, t] = noise

        # Generate noisy labels based on the flipping matrix P
        ...
        assert actual_noise > 0.0
        print('Actual noise %.2f' % actual_noise)
        y_train_ = y_train_noisy

    return y_train_, actual_noise
...
def noisify(dataset='mnist', nb_classes=10, train_labels=None, noise_type=None, noise_rate=0, random_state=0):
    ...
    if noise_type == 'asymmetric' and dataset == 'cifar10':
        train_noisy_labels, actual_noise_rate = noisify_multiclass_asymmetric_cifar10(train_labels,
                                                                                        noise=noise_rate,
                                                                                        random_state=0,
                                                                                        nb_classes=nb_classes)
    if noise_type == 'asymmetric' and dataset == 'cifar100':
        train_noisy_labels, actual_noise_rate = noisify_multiclass_asymmetric_cifar100(train_labels,
                                                                                            noise=noise_rate,
                                                                                            random_state=0,
                                                                                            nb_classes=nb_classes)    
