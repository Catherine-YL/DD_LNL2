...
class Synthesizer():
    ...
    def test(self, args, val_loader, logger, bench=True):
        ...
        convnet_result, last_result = test_data(args, loader, val_loader, test_resnet=False, logger=logger)

        return convnet_result, last_result
...
def condense(args, logger, device='cuda'):
    ...
    wandb.init(sync_tensorboard=False,
        project=f"DANCE_{args.dataset}",
        job_type="CleanRepo",
        name=f"ipc:{args.ipc}_{args.noise_type}_{args.noise_rate}" ,
        config=args,
        )
    ...
    n_iter = args.niter # if not imagenet: 10000
    it_log = 20
    ...
    for it in range(n_iter):
        ...
        if it % it_log == 0:
            ...
            wandb.log({'Inter_Loss': calib_loss_total / nclass / args.iter_calib}, step=it)
            wandb.log({'Inner-loss': loss_total/nclass}, step=it)

        save_best = 0
        if (it + 1) in it_test:
            conv_result, last_result = synset.test(args, val_loader, logger)

            wandb.log({'best_result': conv_result}, step=it)
            wandb.log({'last_result': last_result}, step=it)
            ...
            if conv_result > best_convnet:
                ...
                wandb.log({'Best Result using': best_convnet}, step=it)
...
if __name__ == '__main__':
    ...
    noise_type_map = {'worst': 'worse_label', 'aggre': 'aggre_label', 'rand1': 'random_label1', 'rand2': 'random_label2', 'rand3': 'random_label3', 'clean100': 'clean_label', 'noisy100': 'noisy_label', 'noisy100_coarse': 'noisy_coarse_label'}
    ...
    if args.is_annot == True:
        ...
        if args.noise_path is None:
            if args.dataset == 'cifar10':
                args.noise_path = './dataSolu/CIFAR-10_human.pt'
            elif args.dataset == 'cifar100':
                args.noise_path = './dataSolu/CIFAR-100_human.pt'
            else: 
