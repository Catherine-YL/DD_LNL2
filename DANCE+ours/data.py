...
class ClassDataLoader(MultiEpochsDataLoader):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.cls_targets = torch.tensor([np.ones(self.batch_size, dtype=np.int64) * c for c in range(self.nclass)],
                                        dtype=torch.long,
                                        requires_grad=False,
                                        device='cuda')
...
class ClassMemDataLoader():
    ...
    def __init__(self, dataset, batch_size, drop_last=False, device='cuda'):
        ...
        self.class_sampler = ClassBatchSampler(self.cls_idx, self.batch_size, drop_last=True)
        self.cls_targets = torch.tensor([np.ones(batch_size, dtype=np.int64) * c for c in range(self.nclass)],
                                        dtype=torch.long,
                                        requires_grad=False,
                                        device=self.device)
    ...
    def class_sample(self, c, ipc=-1):
        if ipc > 0:
            if len(self.cls_idx[c]) < ipc:
                indices = self.cls_idx[c] + np.random.choice(self.cls_idx[c], ipc - len(self.cls_idx[c]), replace=True).tolist()
            else:
                indices = self.cls_idx[c][:ipc]
        else:
            indices = next(self.class_sampler.samplers[c])
...
class ClassPartMemDataLoader(MultiEpochsDataLoader):
    ...
    def __init__(self, subclass_list, real_to_idx, *args, **kwargs):
        ...
        self.class_sampler = ClassBatchSampler([self.cls_idx[c] for c in subclass_list],
                                               class_batch_size,
                                               drop_last=True)
        self.cls_targets = torch.tensor([np.ones(class_batch_size, dtype=np.int64) * c for c in range(self.nclass)],
                                        dtype=torch.long,
                                        requires_grad=False,
                                        device='cuda')
