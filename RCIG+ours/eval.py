...
def get_config():
    ...
    config.kernel.eval_batch_size = 500

    return config


def main(dataset_name = 'cifar10', data_path=None, zca_path=None, train_log=None, train_img=None, width=128, depth=3, normalization='identity', eval_lr = 0.0001, random_seed=0, message = 'eval_log', output_dir = None, max_cycles = 1000, config_path = None, checkpoint_path = None, save_name = 'eval_result', log_dir = None, eval_arch = 'conv', models_to_test = 5, is_coarse=False):
    # --------------------------------------
    # Setup
    # --------------------------------------
    ...
    absolute_path = os.path.abspath(f'./{checkpoint_path}')
    loaded_checkpoint = checkpoints.restore_checkpoint(absolute_path, None)
    ...
    for g in range(models_to_test):
        ...
        if not use_batchnorm:
            ...
            for g in range(num_online_eval_updates//500):
                acc = algorithms.eval_on_test_set(bum, ds_test, has_bn = False, centering = False)
                print(f'train checkpoint {(g) * 500} acc {acc}')
                if acc >= max_acc:
                    max_acc = acc
    ...
    logging.info('with data augmentation max_acc_avg: {:.2f} pm {:.2f}'.format(100 * np.mean(max_accs), 100 * np.std(max_accs)))
    print('with data augmentation avg: {:.2f} pm {:.2f}'.format(100 * np.mean(accs), 100 * np.std(accs)))
    print('with data augmentation max_acc_avg: {:.2f} pm {:.2f}'.format(100 * np.mean(max_accs), 100 * np.std(max_accs)))
