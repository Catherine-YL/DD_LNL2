...
def run_rcig(coreset_images_init, coreset_labels_init, net_forward_init, net_forward_apply, train_loader, alg_config, key, inner_learning_rate, hvp_learning_rate, test_fn = None, coreset_train_state = None, pool = None, lr_tune = False, start_iter = 0):
    ...
    if coreset_train_state is None:
        ...
        coreset_init_params = proto_obj.init({'params': key})['params']
    ...
    if start_iter == 0 and not lr_tune and alg_config.output_dir is not None:
        ...
        checkpoints.save_checkpoint(ckpt_dir = absolute_path, target = coreset_train_state, step = 0, keep = 1e10)
    ...
    while(n_steps < alg_config.max_steps):
        for train_images, train_labels in train_loader.as_numpy_iterator():
            ...
            if outer_acc > max_outer_acc:
                max_outer_acc = outer_acc
            ...
            if start_iter + n_steps in alg_config.checkpoint_iters and not lr_tune and n_steps != 0 and alg_config.output_dir is not None:
                ...
                absolute_path = os.path.abspath('./{}/'.format(alg_config.output_dir))
                checkpoints.save_checkpoint(ckpt_dir = absolute_path, target = coreset_train_state, step = n_steps + start_iter, keep = 1e10)
    ...
    return coreset_train_state, key, model_pool, inner_learning_rate, hvp_learning_rate, max_outer_acc
...
@functools.partial(jax.jit, static_argnames=('has_bn', 'net_forward_apply', 'coreset_train_state_apply', 'aug', 'do_precompute', 'max_forward_batch_size', 'use_x64'))
...
@functools.partial(jax.jit, static_argnames=('has_bn', 'self_loss', 'use_base_params', 'max_forward_batch_size', 'use_x64'))
