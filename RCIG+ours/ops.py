...
coarse_label_mapping = tf.constant([
    4,  # apple -> fruit_and_vegetables
    1,  # aquarium_fish -> fish
    15, # baby -> people
    8,  # bear -> large_carnivores
    12, # beaver -> aquatic_mammals
    6,  # bed -> household_furniture
    7,  # bee -> insects
    7,  # beetle -> insects
    18, # bicycle -> vehicles_1
    4,  # bottle -> food_containers
    4,  # bowl -> food_containers
    15, # boy -> people
    9,  # bridge -> large_man-made_outdoor_things
    18, # bus -> vehicles_1
    7,  # butterfly -> insects
    11, # camel -> large_omnivores_and_herbivores
    4,  # can -> food_containers
    9,  # castle -> large_man-made_outdoor_things
    7,  # caterpillar -> insects
    11, # cattle -> large_omnivores_and_herbivores
    6,  # chair -> household_furniture
    15, # chimpanzee -> large_omnivores_and_herbivores
    5,  # clock -> household_electrical_devices
    10, # cloud -> large_natural_outdoor_scenes
    7,  # cockroach -> insects
    6,  # couch -> household_furniture
    13, # crab -> non-insect_invertebrates
    17, # crocodile -> reptiles
    4,  # cup -> food_containers
    17, # dinosaur -> reptiles
    0,  # dolphin -> aquatic_mammals
    11, # elephant -> large_omnivores_and_herbivores
    1,  # flatfish -> fish
    10, # forest -> large_natural_outdoor_scenes
    12, # fox -> medium_mammals
    15, # girl -> people
    12, # hamster -> small_mammals
    9,  # house -> large_man-made_outdoor_things
    11, # kangaroo -> large_omnivores_and_herbivores
    5,  # keyboard -> household_electrical_devices
    5,  # lamp -> household_electrical_devices
    19, # lawn_mower -> vehicles_2
    8,  # leopard -> large_carnivores
    8,  # lion -> large_carnivores
    17, # lizard -> reptiles
    13, # lobster -> non-insect_invertebrates
    15, # man -> people
    18, # maple_tree -> trees
    2,  # motorcycle -> vehicles_1
    10, # mountain -> large_natural_outdoor_scenes
    12, # mouse -> small_mammals
    4,  # mushroom -> fruit_and_vegetables
    18, # oak_tree -> trees
    4,  # orange -> fruit_and_vegetables
    3,  # orchid -> flowers
    0,  # otter -> aquatic_mammals
    18, # palm_tree -> trees
    4,  # pear -> fruit_and_vegetables
    19, # pickup_truck -> vehicles_2
    18, # pine_tree -> trees
    10, # plain -> large_natural_outdoor_scenes
    4,  # plate -> food_containers
    3,  # poppy -> flowers
    12, # porcupine -> medium_mammals
    12, # possum -> medium_mammals
    12, # rabbit -> small_mammals
    12, # raccoon -> medium_mammals
    1,  # ray -> fish
    9,  # road -> large_man-made_outdoor_things
    19, # rocket -> vehicles_2
    3,  # rose -> flowers
    10, # sea -> large_natural_outdoor_scenes
    0,  # seal -> aquatic_mammals
    1,  # shark -> fish
    12, # shrew -> small_mammals
    12, # skunk -> medium_mammals
    9,  # skyscraper -> large_man-made_outdoor_things
    13, # snail -> non-insect_invertebrates
    17, # snake -> reptiles
    13, # spider -> non-insect_invertebrates
    12, # squirrel -> small_mammals
    18, # streetcar -> vehicles_1
    3,  # sunflower -> flowers
    4,  # sweet_pepper -> fruit_and_vegetables
    6,  # table -> household_furniture
    19, # tank -> vehicles_2
    5,  # telephone -> household_electrical_devices
    5,  # television -> household_electrical_devices
    8,  # tiger -> large_carnivores
    19, # tractor -> vehicles_2
    18, # train -> vehicles_1
    1,  # trout -> fish
    3,  # tulip -> flowers
    17, # turtle -> reptiles
    6,  # wardrobe -> household_furniture
    0,  # whale -> aquatic_mammals
    18, # willow_tree -> trees
    8,  # wolf -> large_carnivores
    15, # woman -> people
    13  # worm -> non-insect_invertebrates
])
...
def load_data(ds, img_shape, preprocess_op, resolution=32, is_fineTocoarse=False, batch_size=1000):
    size = len(ds)
    ...
    # fine -> coarse
    if is_fineTocoarse:
        ds = ds.map(lambda x, y: (x, tf.gather(coarse_label_mapping, y)), num_parallel_calls=tf.data.AUTOTUNE)
