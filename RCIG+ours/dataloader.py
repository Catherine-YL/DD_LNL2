...
def get_dataset(config, is_coarse, return_raw=False):
    dataset_name = config.name
    data_path = config.data_path
    zca_path = config.zca_path
    zca_reg = config.zca_reg
    is_fineTocoarse = False
    ...
    if is_coarse == True and dataset_name == 'cifar100':
        is_fineTocoarse = True
        num_classes, class_names = ds_builder.info.features['coarse_label'].num_classes, ds_builder.info.features['coarse_label'].names 
    else:
    ...
    if dataset_name in ['imagenet_resized/64x64', 'caltech_birds2011']:
        ...
        x_train, y_train = load_data(ds_train, img_shape, preprocess_op, resolution, is_fineTocoarse, batch_size=5000)
        x_test, y_test = load_data(ds_test, img_shape, preprocess_op, resolution, is_fineTocoarse, batch_size=5000)
