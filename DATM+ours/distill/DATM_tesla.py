...
def main(args):
    ...
    if args.pix_init == "samples_predicted_correctly":
        ...
        temp_logits = None
        for select_times in range((len(image_syn) + batch_size - 1) // batch_size): 
            current_data_batch = image_syn[batch_size * select_times: batch_size * (select_times + 1)].detach().to(args.device)
            batch_logits = Temp_net(current_data_batch, flat_param=Initialize_Labels_params).detach()

            if temp_logits is None:
                temp_logits = batch_logits
            else:
                temp_logits = torch.cat((temp_logits, batch_logits), 0)
        
        Initialized_Labels = temp_logits  
        print(f"Shape of Initialize_Labels_params: {Initialize_Labels_params.shape}")
        print(f"Shape of image_syn: {image_syn.shape}")
