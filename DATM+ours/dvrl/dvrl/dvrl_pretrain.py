...
def pretrain(predictor_model, data_loader, inner_learning_rate, feat_model, weights=None, cuda='cpu', epoch=5):
    """
    ...
    for epoch in range(epoch):
        ...
        for i_batch, datum in enumerate(data_loader):
            ...
            image_batch, label_batch = datum[0].float().to(cuda), datum[1].long().to(cuda)
            # import pdb
            # pdb.set_trace()

            feature_batch = feat_model(image_batch, finaly_layer=False)
            ...
            if i_batch % 10:
                ...
                print("[epoch %d][%d/%d], total loss: %.4f, accumulated accuracy: %.4f"
                      % (epoch + 1, i_batch + 1, len(data_loader), loss.item(), corr_num / total_num))
